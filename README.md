...
3. **多言語・声色のランダム化**(任意): `--voice random` を指定すると、
   [tts/tts_extreme専用] 英語を含む24言語・すべての言語で男性/女性ボイスも
...
`--voice random` を指定すると、英語を含む以下の24言語・全言語で男性/女性
の両方のボイスからランダムに選ばれます。
...
- **フォルマントバリアント**: 上記以外の19言語は、espeak-ng本体に内蔵さ
...
24言語のうち13言語が「実際の文字体系」を使う言語(後述)で、残る11言語が
...
上記11言語は「土台の文字(ラテン文字・Zalgo)自体は変えず、読み上げ言語だ
