...
3. **多言語・声色のランダム化**(任意): `--voice random` を指定すると、
   [tts/tts_extreme専用] 英語を含む29言語・すべての言語で男性/女性ボイスも
...
`--voice random` を指定すると、英語を含む以下の29言語・全言語で男性/女性
の両方のボイスからランダムに選ばれます。
...
- **フォルマントバリアント**: 上記以外の21言語は、espeak-ng本体に内蔵さ
...
29言語のうち13言語が「実際の文字体系」を使う言語(後述)で、残る16言語が
