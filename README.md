...
`--voice random` を指定すると、英語を含む以下の13言語からランダムに選ば
れます。
...
### 実際の文字体系を使う言語(ロシア語・ジョージア語・タイ語)

上記10言語は「土台の文字(ラテン文字・Zalgo)自体は変えず、読み上げ言語だ
けを変える」機能でしたが、ロシア語・ジョージア語・タイ語の3言語は**単語
自体をその言語の実際の文字からランダムに生成**します
(`word_generator.random_script_word()` / `random_thai_word()`)。西洋圏の
視聴者にとって文字自体が読めない、より直接的な「発音不能」感を狙ってい
ます。実在する単語ではなく、各言語の文字をランダムに組み合わせた造語です。

- **ロシア語・ジョージア語**: キリル文字/ジョージア文字をランダムに選び、
  一定確率で同じ文字を連打する(子音クラスターのような見た目を作る)、
  Zalgo単語の生成ロジックと似た方式です。Zalgoの結合文字・装飾記号は
  使いません(英語音声での無音確認しか取れていないため)。
- **タイ語**: タイ文字は子音字+母音記号(前後左右に付く、単体では使わな
  い)で音節を作る文字体系のため、専用のロジック(`random_thai_word()`)
  で子音→(確率的に)母音記号、を繰り返して音節列を作ります。子音字だけ
  を並べるとタイ語の正書法として不完全な文字列になってしまうため、
  詳細は「ハマった罠」の12番を参照してください。

対応言語はChromium+Notoフォントでの描画、espeak-ngでの音声合成の両方が
実機で正常に動作することを確認済みの3言語のみです。
