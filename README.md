...
動画フレーム(`frame_builder.py`)は元々白背景+黒文字のミニマルなデザイン
だったが、YouTube Studioの動画一覧のような小さいサムネイル表示だと、全
動画がほぼ同じ白い長方形にしか見えず視認性に欠けるという指摘があった。

対策として、フレームの背景をダーク(`#131417`)にし、単語の文字色を
**生成モードごとのアクセントカラー**(`config.MODE_ACCENT_COLORS`)にした:

| モード | 文字色 |
|---|---|
| `tts` | シアン `#5fd0e0` |
| `tts_extreme` | マゼンタ `#ef5da8` |
| `glitch` | イエロー `#f3d94a` |

言語ではなくモード単位で色分けしているのは、`generate.py`が
`build_frame()`を呼ぶ時点で言語(23言語)よりモード(3種類)の方が扱い
やすく、かつ色の絞り込みとしても十分だと判断したため。キッカー
("How to Pronounce")とサブテキストはダーク背景でも読める薄いグレー
(`#8d9199` / `#6e727a`)、スピーカーアイコンはオフホワイト
(`#f2f1ea`)にそれぞれ変更している。
