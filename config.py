...
# 判断から、一時的に均等抽選(各1/3)より比率を下げていた(tts:2,
# tts_extreme:2, glitch:1)。しかしyoutube_analytics.pyでの実測で、主指標
# (投稿からの経過日数で正規化した1日あたり再生数)は2回連続でglitchが
...
# 画面は未検証(「テスト」ステータス)のまま運用しているため、リクエストが
# 断続的に権限不足(403 forbidden)で拒否されることを実機で確認した(3回中
# 2回失敗、成功時と失敗時でコードは同一)。失敗しても動画本体の投稿は止まら
# ない設計だが、失敗時もcaptions.insertのクォータ(400 units)を無駄に消費
# するため、信頼性が改善する(Googleのアプリ検証を通す等)まで一時的に無効化
